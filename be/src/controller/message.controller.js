...
const getMessages = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({message: "Could not get messages", error: error.message});
    }
}

const newMessage = async (req, res) => {
    const senderId = req.userId;
    const {conversationId, content, messageType, fileUrl, fileName, clientOffset} = req.body;
    try {
        const message = await messService.newMessage(clientOffset, conversationId, senderId, content, messageType, fileUrl, fileName);
